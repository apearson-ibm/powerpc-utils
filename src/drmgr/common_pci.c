 ...
 */
static int
create_vio_nodes(const char *ofdt_path, struct dr_connector *drc_list,  struct dr_node **node_list)
{
	DIR *d;
	struct dirent *de;
	char child_path[DR_PATH_MAX];
	...
	while ((de = readdir(d)) != NULL) {
		...
		snprintf(child_path, DR_PATH_MAX, "%s/%s", ofdt_path, de->d_name);
		if (get_my_drc_index(child_path, &my_drc_index))
			continue;
		drc = NULL;
		for (drc_search = drc_list; drc_search != NULL; drc_search = drc_search->next) {
		...
		if (dev_type == VIO_DEV)
		{
			...
			while (node_search != NULL)
			{
				if (drc->index == node_search->drc_index) {
					// Use this node - and remove it from the list
					node = node_search;
					if (node_search_prev != NULL)
						node_search_prev->next = node->next;
					else
